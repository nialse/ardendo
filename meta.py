...
def load_models(models_csv, provider):
    ...
    with Path(models_csv).open(newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            ...
            row_think = (row.get("think") or "auto").strip().lower()
            if not row_provider or not row_name:
                continue
            if row_think not in {"auto", "true", "false"}:
                print(f"Unsupported think value for {row_name}: {row_think}", file=sys.stderr)
                raise SystemExit(2)
            ...
            rows.append(
                {
                    ...
                    "model": row_name,
                    "think": row_think,
                    "label": model_label(row_name, row_think),
                }
            )
    return rows
...
def load_models_by_provider(models_csv, provider):
    ...
    for model in load_models(models_csv, provider):
        if model["provider"] in selected:
            selected[model["provider"]].append(model)
    return {name: models for name, models in selected.items() if models}


def resolve_models_csv(models_csv):
...
def build_report(models_csv, provider, turns, timeout_s, run_dir):
    return {
...
def run_ardendo(provider, model, think, label, turns, progress_path, request_timeout, timeout_s=0, log_path=None):
    cmd = [
        str(python),
        str(root / "ardendo.py"),
        "--provider",
    ...
    try:
        if log_path:
            ...
            log_handle.write(f"\n[{time.strftime('%Y-%m-%dT%H:%M:%S%z')}] start {provider} {label}\n")
    ...
    completed_after, refusals_after = read_counts(progress_path, label)
    if timed_out:
...
smoke_parser.add_argument("--models-csv", default=str(default_models_csv))
...
if args.command == "run":
    models_csv = resolve_models_csv(args.models_csv)
    ...
    report = build_report(models_csv, args.provider, args.turns, args.timeout, run_dir)

    failed = []
    for provider, models in selected.items():
        for model in models:
            progress_path = model_progress_path(run_dir, provider, model["label"])
            log_path = model_log_path(run_dir, provider, model["label"])

            if args.restart:
                remove_paths([progress_path, log_path])

            completed_before, refusals_before = read_counts(progress_path, model["label"])
            if completed_before >= args.turns:
                ...
                print(f"SKIP {provider} {model['label']} ({completed_before}/{args.turns})")
            ...
            print(f"{'OK' if result['ok'] else 'FAIL'} {provider} {model['label']} ({result['seconds']}s)")
            if not result["ok"]:
...
if args.command == "smoke":
    ...
    for model in rows:
        ...
        progress_path = progress_dir / f"progress_smoke_{provider}_{safe_name(model['label'])}.json"
        if args.restart:
            remove_paths([progress_path])

        data_points, refusals = read_counts(progress_path, model["label"])
        if data_points >= args.turns:
            ...
            report["results"].append(result)
            print(f"SKIP {provider} {model['label']} ({data_points}/{args.turns})", file=sys.stderr)
            continue
        ...
        report["results"].append(result)
        print(f"{'OK' if result['ok'] else 'FAIL'} {provider} {model['label']} ({result['seconds']}s)", file=sys.stderr)
    ...
    for result in report["results"]:
        lines.append(
            ...
            f"think={result.get('think', 'auto')}\tdata={result['data_points']}\t"
            f"refusals={result['refusals']}\tseconds={result['seconds']}\t{result['error'] or ''}"
        )
...
if args.command == "probe":
    ...
    models_csv = resolve_models_csv(args.models_csv)
    selected = [model["model"] for model in load_models(models_csv, "openrouter")]
...
if args.command == "sync":
    models_csv = resolve_models_csv(args.models_csv)
    desired = sorted({model["model"] for model in load_models(models_csv, "ollama")})
...
if args.command == "install-ssh":
    if not args.host:
        print("Missing host. Usage: python meta.py install-ssh ollama.example.org or set OLLAMA_SSH_HOST", file=sys.stderr)
        raise SystemExit(2)
